...
- 우린 이제 URL 동적 라우팅에 대해 얘기해볼거야
- URL은 우리의 쿼리-DB와 상호관계가 있고 이건 정말 비슷해야하고 제일 최고의 방법은 admin을 체크하는 거야
- 만약 우리가 admin에 들어가서 posts를 본다면 어떤 posts든 보면 나는 admin posts를 볼 수 있다
- 관련있는 ID를 가진 숫자를 볼 수 있고 이건 다른 URL로 바뀐다
- 이건 URL의 다른 부분이고 이건 중요해
- 13 포스트는 변경되고 포스트 디테일을 볼 수 있어
- 우린 view로 들어가서 ID를 변경하고 저장하고 새로고침하고 이걸 다시 볼 수 있어
- 우리가 URL을 수정하려면 우리는 url.py로 들어가고
- 추가할 수 있어 새로운 우리의 디테일 URL을
- 코드를 수정해 이건 ID의 새로운 패러미터고 이 표현만 받아들인다는 뜻이야 우리는 slash를 넣어야하고
- 우리 표현이 좀 어려운데 하나의 값만 통과해서 이 곳에 도착한다는 거야
- ID가 같으면 새로고침하고 들어왔을 때 출입을 받아들일 거야
- 만약에 내가 ID를 바꾼다면 다시 안될거
- view로 다시 돌아가서 변경하고 
- pk는 다른 옵션이야 PK나 ID는 좋은 구분이 되지
- 만약 ID를 변경하고 볼 수 없고 13으로 다시 등록해서 들어가면 페이지를 확인할 수 있어
- 일반적인 URL 표현들은 github가이드가 있고 볼 수 있어 common django URL regular expression patterns이
- 오브젝트 아이디나 이름 등 다양한 걸 볼 수 있어 
- url pattern들은 모든 url 표현들을 체킹해
- 키워드 인자들은 패스되고 view안으로 들어가서 query가 있어

* * *
## 19/38 URL Links & Get Absolute URL
- object list안으로 들어가자
- title에 링크를 추가하고 posts/object.id 형태로 변경한다
- 내가 가진 모든 링크들은 클릭하면 내가 원하는 곳으로 가게 해준다
- 인덱스로 들어가서 수정하고 다시 새로고침해도 작동한다
- 이것의 가장 큰 이점은 이렇게 사용했을 때 우리가 URL로 들어가면 여기 디테일로 돌아갈 수 있다
- URL을 하드코딩하는 것보다 훨씬 편하고 좋다
- models.py로 들어가서 함수를 추가한다 인덱스로 다시 돌아가서 컷하고 저장하고 새로고침한다
- 이건 날 posts로 들어가게 하지만 만약 우리가 get_absolute_url을 보고있다면 이건 크게 다르지 않다 우리가 하드코딩했던 거
- 우리는 이거랑 비슷한 게 필요하다 우린 그걸 reverse라고 부른다 
- reverse는 진짜 좋아 reverse는 키보드 인자가 id가 어쩌구이면 URL로 들어가고나서 바꿔주고 view로 들어가서 이것들을 ABC로 바꿔준다
- 우린 또 우리의 model로 들어가서 우리의 reverse를 부르고 또 ABC로 바꾸거나 키보드인자의 이름을 바꿀 수 있다
- URL name detail을 유지할 수 있는데 만약 우리가 다양한 apps에 같은 URL이름을 할당하면 안된다 에러남
- url namespaces는 URL을 세팅할 때 적용된다
- 충돌하면 바꿔야한다


* * *
## 20/38 Model Form & Create View
- 우린 이제 뷰부분이 다 끝난 거야
- 좀 더 쉬운 걸 하자 model form이란 거야
- 모델 폼이라는 건 기본적으로 웹 페이지를 채우고 이것이 db에 데이터를 저장하는 것이다
- forms.py라는 파일을 만들고 PostForm이라는 클래스를 만들어
- 템플릿에 post_form.html이라는 파일을 만들고 index.html을 복사해서 붙여넣어 수정하고
- 우린 이미 URL을 가지고 있어 create에
- 127.0.0.1:8000/posts/create/에 접속하면 우린 이젠 폼을 볼 수 있어
- 엄청나게 간단해
- 아직 인풋 버튼을 만들진 않았으니까 넣어볼
- 이제 submit 버튼을 볼 수 있어
- 지금은 Title과 Content에 내용을 쓰고 submit 버튼을 누르더라도 아무일도 일어나지 않아
- 이건 당연한 거 폼을 wrap하지는 않았거든
- post_form.html에 form 태그를 만들고 다시 새로고침을 하면 create_post가 타이틀과 컨텐트를 넣었다는 URL 바를 볼 수 있어
- 이건 왜냐면 폼을 위한 기본 메소드기 때문이야
- 다시 내용을 채워서 보내면 CSRF verification 실패라고 해 왜냐하면 인증이 필요하다는 거야 너의 웹사이트는 post data를 보낼 수 없으니까
- 장고는 기본 보안을 제공해 html파일 을 수정해서 csrf_token을 추가해
- csrf는 장고 공식문서에서 볼 수 있어
- 이번에는 데이터가 갔을 거야
- 데이터가 아직 captured 되지않아서 그걸 해줘야해
- 터미널을 확인해보면 빈 딕셔너리와 csrf token이 갔다는 메시지를 볼 수 있어
- 이건 django forms의 이유야
- model form은 장고 밸리데이션을 가지고 있어
- post_create중에서 오늘 만든 걸 주석처리하고 request.POST를 추가하자
- 다시 새로고침하고 내용을 써서 보내면 아무 일도 일어나지 않아
- instance.save를 추가하고 다시 동작하면 /posts/ 페이지 가장 밑에 추가된 게 보일거야
- form에 벨리데이션이 있다면 동작하고 db에 데이터가 저장된다는 거지
- 우린 오늘 model form은 벨리데이션을 갖고 있다는 걸 봤어
- 간단하지? 우린 오늘 모델을 만들었고 모델에 모델폼을 추가했고 html에도 추가했고 우리의 뷰가 모든 것을 다루는 걸 봤어
